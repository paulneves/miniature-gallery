...
The live GitHub repository is authoritative. Preserve historical Masters. v1.6.1 supersedes conflicting workflow instructions in older versions. Direct ChatGPT Work → GitHub binary intake is the validated normal Phase A route. v1.6.1 adds unique temporary Phase A package prefixes so multiple models with identical titles never require content comparison merely to distinguish staging packages.
...
Format:
`<HHMMSS>-<slug>-<role>.jpg`

Example package started at 19:13:27:
...
A second same-title package such as `191542-imperial-remnant-trooper-mini.jpg` may independently become `SW-0096-imperial-remnant-trooper-mini.jpg`.
...
- Manifest title preserves the supplied title and does not need to be unique.
...
6. Capture one `HHMMSS` temporary package prefix and create the normalized slug. Do not allocate permanent catalogue ID.
7. Verify exact package key `<HHMMSS>-<slug>` does not already exist as a manifest in `uploads/`. Do not treat another prefix with the same slug/title as a duplicate.
8. Ensure Drive path exists.
9. Archive renamed originals to Drive unchanged using the same package prefix and validate their presence.
10. Generate and validate staging web JPGs programmatically using the same package prefix.
11. Create and validate `<HHMMSS>-<slug>.json`, including mandatory `paintBrand` and exact prefixed staging asset filenames.
12. Persist all staging web assets directly to GitHub `uploads/` using ChatGPT Work capabilities.
13. Persist manifest to `uploads/` last.
14. Re-read/inspect GitHub and validate manifest plus every declared asset.
15. Report Phase A result, including temporary package key.
16. **STOP. Do not execute Phase B automatically.**
...
10. Determine normalized slug from manifest title for final filename readability; do not use title/slug as identity.
11. For v1.6.1 staging packages, remove the temporary six-digit `HHMMSS-` package prefix when constructing final names. Never carry the staging time prefix into final web filenames or catalogue paths.
12. Destination: `images/<universe>/<faction>/<subfaction>/`.
13. Move/reuse staged assets into final filenames `<id>-<slug>-<role>.jpg`.
14. Add miniature to `catalog.json` using permanent global `id`, explicit `paintBrand`, and final ID-prefixed image paths.
15. Update top-level `lastCatalogNumber` to highest successfully assigned numeric ID as part of successful publication.
16. Preserve unrelated records and allow duplicate names/titles.
17. Catalogue paths reference final `images/...`, never `uploads/...`.
18. Validate catalogue IDs against `^[A-Z]{2}-\d{4}$`, prefix/universe mapping, unique full IDs, globally unique numeric suffixes, final assets and valid JSON.
19. Validate `lastCatalogNumber` equals the highest allocated numeric suffix after publication.
20. Validate every referenced final file exists, begins with the record ID, and contains no temporary `HHMMSS-` package prefix.
21. Only after successful asset placement and catalogue/counter update remove source staging assets belonging to that manifest/package.
22. Delete that package manifest last.
23. On failure retain recoverable assets/manifest and do not falsely advance counter for an unpublished model.
24. Verify GitHub Pages where practical, including that ID appears in detail viewer and not gallery cards.
25. Report successful, still pending, blocked/incomplete, allocated IDs and commits where applicable.

### Multiple pending models
For a batch, use deterministic manifest/package processing order and allocate consecutive global numbers. Duplicate titles are processed independently. Never deduplicate by title or slug. Re-read/reconcile repository state as necessary before committing.

### Concurrent Phase B safety
If repository state changes between reading `lastCatalogNumber` and committing publication, do not force or reuse stale allocation. Re-read newest repository state, recompute next global number, rebuild change and commit safely. Two publications must never receive the same numeric component even if universe prefixes differ.

## QUEUE INVARIANT
`uploads/` is the strict Phase A → Phase B boundary. A successfully completed Phase A leaves a validated uniquely prefixed manifest and its declared assets in `uploads/`. A successfully completed Phase B leaves final ID-prefixed assets, valid catalogue entry and updated counter, with no source assets/manifest remaining for that package. Missing `uploads/` means empty queue.

## SAFE GITHUB RULES
Never force `main`. Re-read current state before SHA-dependent writes. Preserve unrelated concurrent changes. Prefer logically atomic Phase B publication when capabilities permit. Reuse valid blobs/files when moving. On conflict/non-fast-forward rebuild against newest repository state. Direct binary intake through ChatGPT Work is the validated normal route.
...
`id` format is `<two-letter-universe-prefix>-<four-digit-global-number>` and is the canonical permanent key. Names may repeat. Final image filenames include ID prefix so same-name miniatures cannot overwrite one another. Temporary Phase A time prefixes never appear in final catalogue paths.
...
v1.6.1 is a patch to v1.6.0. It preserves the two-phase ChatGPT Work architecture and adds a mandatory temporary `HHMMSS-` Phase A package prefix to eliminate staging collisions and unnecessary duplicate validation for models sharing the same title. Phase B removes that temporary prefix and replaces it with the permanent catalogue ID. Drive originals retain the temporary prefix to prevent archive collisions.
...
- Every new Phase A package gets one unique six-digit `HHMMSS` temporary prefix shared by manifest and all its files.
- Same-title packages with different temporary prefixes are separate packages; do not compare content merely because titles match.
- Never overwrite or merge an existing package based on title/slug.
...
- Phase B removes temporary `HHMMSS-` prefix and creates final `<id>-<slug>-...` filenames.
- Temporary Phase A prefix must never appear in final catalogue paths.
...
- Drive originals unchanged except filename; v1.6.1 Drive originals retain their temporary prefix to prevent same-title collision.
...
**Version:** 1.6.1  
**Purpose:** Autonomous two-phase Miniature Gallery workflow using direct ChatGPT Work → GitHub intake, uniquely time-prefixed Phase A staging packages, Google Drive original archival, global catalogue IDs, manifest-based pending models and safe Phase B publication.
